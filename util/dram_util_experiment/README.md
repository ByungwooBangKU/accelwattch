...
reports/nvidia_geforce_rtx_3090_202605141739/<tag>_rep1_ncu_*.ncu-rep   # --ncu-profile 기본값
...
이 명령은 정상 NVML power 반복 실험을 끝낸 뒤 별도 validation process로 `run_pjbit_ncu.sh`를 호출한다. 기본값은 `--ncu-repeat-scope rep1`이므로 repeat 전체를 다시 profiling하지 않고, rep1 대표 조건에 대해서만 `read`와 각 write pattern을 따로 validation한다. 결과는 같은 실험 디렉터리의 `<tag>_rep1_ncu_*.ncu-rep`에 저장된다.
