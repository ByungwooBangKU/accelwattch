...
reports/nvidia_geforce_rtx_3090_202605141739/<tag>_ncu_*.ncu-rep        # --ncu-profile 또는 --ncu-only 사용 시
...
이 명령은 정상 NVML power 반복 실험을 끝낸 뒤 별도 validation process로 `run_pjbit_ncu.sh`를 호출한다. `read`와 각 write pattern을 따로 실행하며, 기본적으로 각 process에서 warmup 1회와 calibration 1회를 `--ncu-launch-skip 2`로 건너뛰고 active 100% phase의 첫 kernel launch 1개만 profile한다. 결과는 같은 실험 디렉터리의 `<tag>_ncu_*.ncu-rep`에 저장된다.
