...
| ID | 연산 | 수식 | 메모리 I/O | 연산 intensity |
|---|---|---|---|---|
| `fp16_mul` | element-wise multiply | `y = a · b` | 2R + 1W | 1 FLOP/3 words |
| `fp16_add` | element-wise add | `y = a + b` | 2R + 1W | 1 FLOP/3 words |
| `fp16_softmax` | row-wise softmax | `y_i = exp(x_i)/Σ exp(x_j)` | ~2R + 1W (2pass) | ~5 FLOP/elem |
| `fp16_gelu` | GeLU activation | `y = 0.5·x·(1+tanh(…))` | 1R + 1W | ~8 FLOP/elem |
| `fp16_layernorm` | layer normalization | `y = (x−μ)/σ · γ + β` | 2R + 1W (2pass) | ~5 FLOP/elem |
| `fp8_mul` | FP8 multiply (cast in/out) | 위와 동일, dtype 만 E4M3 | 2R + 1W | 1 FLOP/3 words |
| `fp8_add` | FP8 add | 동일 | 2R + 1W | 1 FLOP/3 words |
| `fp8_softmax` | FP8 softmax | 동일 | 2R + 1W | ~5 FLOP/elem |
| `fp8_gelu` | FP8 GeLU | 동일 | 1R + 1W | ~8 FLOP/elem |
| `fp8_layernorm` | FP8 LayerNorm | 동일 | 2R + 1W | ~5 FLOP/elem |
...
| ID | dtype | 경로 | A100 | H100 |
|---|---|---|---|---|
| `matmul_fp32_simt` | FP32 | CUDA core SIMT — Tensor Core 미사용 | ✓ | ✓ (참조용) |
| `matmul_tf32_tc` | TF32 | Ampere TC (mma.sync m16n8k8) | ✓ | ✓ |
| `matmul_fp16_tc` | FP16 | Ampere/Hopper TC (mma.sync m16n8k16) | ✓ | ✓ |
| `matmul_bf16_tc` | BF16 | 동일 TC 경로, mantissa만 다름 | ✓ | ✓ |
| `matmul_fp8_te` | FP8 E4M3 | Hopper TC + Transformer Engine wrapper | ✗ | ✓ |
...
| `matmul_fp8_te` | ✗ | ~0.07 pJ/FLOP | — | H100 only |
