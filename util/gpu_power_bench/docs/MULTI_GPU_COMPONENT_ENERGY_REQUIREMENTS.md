...
7. `component_validation_report.py` writes tables to `--out-dir` and image report PNGs to a separate `--image-dir`.
8. Image report filenames are category-numbered:
...
9. Report classification must distinguish:
