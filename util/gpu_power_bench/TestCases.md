...
> **중요 — Standalone vs Fused** : 위 `softmax` / `gelu` / `layernorm` 은 PyTorch **standalone** op 측정값. 실제 LLM 에서는 FlashAttention / fused MLP 안에서 실행 → HBM 트래픽이 사라지고 surrounding matmul 과 latency-hide 됨. fused kernel 안의 op 에너지 추정에는 이 standalone 값을 그대로 쓰면 안 됨. 별도 측정은 **A.5 (fused, opt-in)** 참조.
...
### A.5 Fused vs Standalone (`category = fused`, opt-in)   *(planned — REVIEW.md G11 / P1.4)*
...
| | `linear_gelu` | `torch.compile` 된 `gelu(x @ W + b)` (epilogue fusion) | x + W + b read, y write | 1× | inductor 가 fuse 안 하면 TE `LayerNormMLP` fallback |
...
| flag | 기본 | 비고 |
|---|---|---|
| `--include-fused` | **off** | opt-in. 활성화 시 6 신규 variant 추가 — sweep 셀 ~30% 증가 |
...
1. Fusion 보장 환경 의존 — torch.inductor epilogue fusion 검증 후 fail 시 TE fallback.
